...
public final class IotBenchLoader extends Loader<IotBenchBenchmark> {
  ...
  protected void loadHub(Connection conn, int hubId) {
    try (PreparedStatement hubPrepStmt =
        getInsertStatement(conn, IotBenchConstants.TABLENAME_HUB)) {
      Hub hub = new Hub();
      hub.setHubId(hubId);
  ...
  protected void loadRoom(Connection conn, int roomId, int hubId) {
    try (PreparedStatement roomPrepStmt =
        getInsertStatement(conn, IotBenchConstants.TABLENAME_ROOM)) {
      Room room = new Room();
  ...
  protected void loadDevice(Connection conn, int deviceId, int roomId, int hubId) {
    try (PreparedStatement devicePrepStmt =
        getInsertStatement(conn, IotBenchConstants.TABLENAME_DEVICE)) {
  ...
  protected void loadSensor(Connection conn, int sensorId, int deviceId) {
    try (PreparedStatement sensorPrepStmt =
        getInsertStatement(conn, IotBenchConstants.TABLENAME_SENSOR)) {
