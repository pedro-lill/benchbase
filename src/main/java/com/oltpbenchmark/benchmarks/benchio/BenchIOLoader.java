 ...
 */
 ...
 import com.oltpbenchmark.benchmarks.benchio.pojo.*;
 ...
 public final class BenchIOLoader extends Loader<BenchIOBenchmark> {
         ...
         threads.add(
             new LoaderThread(this.benchmark) {
                 ...
                 public void load(Connection conn) {
                     loadUsers(conn, BenchIOConfig.configUserCount);
                 }
     ...
     protected void loadUsers(Connection conn, int userCount) {
         try (PreparedStatement userPrepStmt = getInsertStatement(conn, BenchIOConstants.TABLENAME_USER)) {
     ...
     protected void loadHub(Connection conn, int hubId) {
         try (PreparedStatement hubPrepStmt = getInsertStatement(conn, BenchIOConstants.TABLENAME_HUB)) {
             Hub hub = new Hub();
             hub.setHubId(hubId);
     ...
     protected void loadRoom(Connection conn, int roomId, int hubId) {
         try (PreparedStatement roomPrepStmt = getInsertStatement(conn, BenchIOConstants.TABLENAME_ROOM)) {
             Room room = new Room();
     ...
     protected void loadDevice(Connection conn, int deviceId, int roomId, int hubId) {
         try (PreparedStatement devicePrepStmt = getInsertStatement(conn, BenchIOConstants.TABLENAME_DEVICE)) {
     ...
     protected void loadSensor(Connection conn, int sensorId, int deviceId) {
         try (PreparedStatement sensorPrepStmt = getInsertStatement(conn, BenchIOConstants.TABLENAME_SENSOR)) {
